...
一个面向《韩国语（1）》入门词表的网页背词应用。它把固定词库、随机闪卡、课程练习、错词单词本、重点例句、口语考试模拟、登录同步和可视化字幕瀑布整合在一起，适合个人复习，也方便继续扩展成更完整的语言学习工具。

线上访问：[korean-vocab-trainer.vercel.app](https://korean-vocab-trainer.vercel.app)

## 功能概览

- **随机闪卡**：从全部 521 个词条中随机抽卡。
...
- **重点例句**：按课程展示教材重点句，也支持登录后保存自己的个性化例句。
- **口语考试模拟器**：内置 168 组去重问答短句，每课 21 组，支持全局或按课随机抽题；另有“听音练习”，用老师录音切成 45 段音频卡，正面只听音频，背面核对样题问句和建议答案。
...
│   ├── data/
│   │   ├── key-sentences.ts        # 重点例句
│   │   ├── oral-exam.ts            # 口语考试 168 组问答题
│   │   ├── oral-listening.ts       # 口语样题听音练习 45 段切片
│   │   ├── vocabulary.json         # 固定词库，随代码发布
│   │   └── *.test.ts               # 数据完整性测试
│   └── lib/                        # 判题、会话、数据库、类型定义
├── .env.example
├── public/audio/oral-listening/     # 老师问句录音切片
├── package.json
└── README.md
```
...
- 词库总量、必填字段、课程范围。
- 重点例句、口语考试题库、听音练习切片的数量、去重、必填字段、课程范围和音频文件存在性。
- 韩文答案空格归一化。
- 中文答案按释义片段匹配。
